...
function restoreGame() {
  try {
    ...
    if (!Number.isInteger(game.trickCardTarget) || game.trickCardTarget < playerCount) {
      game.trickCardsPlayed = 0;
...
function playCards(playerIndex, cards) {
  ...
  if (game.trickCardsPlayed === 0) {
    game.trickCardTarget = cards.length * players.length;
  }
...
function advanceTurn() {
  if (shouldClearTrick()) {
    ...
    game.trickCardsPlayed = 0;
    game.trickCardTarget = players.length;
